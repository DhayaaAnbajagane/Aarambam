...
if __name__ == '__main__':
    ...
    Pk_w_res    = 2.0
    Pk_phi_res  = 0.0
    Pk_mode     = 1 #We have 0 - No res, 1 - Linear res, and 2 - Log res.
    ...
    #in the arguments and puts it in the right text format. We use the "resbasis" one
    #since we want resonances in our model.
    CONFIG = Am.utils.make_config_resbasis(Nmesh = Nmesh, Nsample= Nsample, Nmax = Nmax, Lbox = Lbox, FileBase = 'Aarambam_ics',
                                           OutputDir = outdir, 
                                           ...
                                           Pk_A_res = Pk_A_res, Pk_w_res = Pk_w_res, Pk_phi_res = Pk_phi_res, Pk_mode = Pk_mode)
    
    with open(outdir + '/ResLPTconfig', 'w') as f: f.write(CONFIG)
    ...
    #Now we just run the res-enhanced, basis-enhanced LPT! This comes packaged within Aarambam, so
    #you can just use the executable. You can run it from the command line, but
    #let's do it via a subprocess here.
    sp.run(f"mpirun -np {Nprocs} Aarambam-2LPT-ResBasis {outdir + '/ResLPTconfig'}", shell = True, env = os.environ)
